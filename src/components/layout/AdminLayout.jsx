...
export default function AdminLayout({ children, darkMode, toggleDarkMode }) {
  ...
  const [animatedText, setAnimatedText] = useState(""); // ✅ FIX
  const [showAnimation, setShowAnimation] = useState(false);

  const typingIndex = useRef(0);

  // 🔐 Auth + Welcome Animation
  useEffect(() => {
    const storedUsername = sessionStorage.getItem("username");
    const storedRole = sessionStorage.getItem("role");
    ...
    if (!hasSeenAnimation) {
      ...
      const typingInterval = setInterval(() => {
        if (typingIndex.current <= welcomeText.length) {
          setAnimatedText(welcomeText.slice(0, typingIndex.current));
          typingIndex.current++;
        } else {
  ...
  function startHeaderAnimation(name) {
    ...
    const interval = setInterval(() => {
      if (index <= text.length) {
        setHeaderAnimatedText(text.slice(0, index));
        index++;
      } else {
  ...
  // 📂 Routes
  const routes = [
    { href: "/dashboard/admin", label: "Dashboard", icon: Database, showFor: ["admin", "user"] },
    { href: "/dashboard/quick-task", label: "Quick Task", icon: Zap, showFor: ["admin"] },
    { href: "/dashboard/assign-task", label: "Assign Task", icon: CheckSquare, showFor: ["admin"] },
    { href: "/dashboard/delegation", label: "Delegation", icon: ClipboardList, showFor: ["admin", "user"] },
    { href: "/dashboard/calendar", label: "Calendar", icon: Calendar, showFor: ["admin", "user"] },
    { href: "/dashboard/license", label: "License", icon: KeyRound, showFor: ["admin", "user"] },
    { href: "/dashboard/traning-video", label: "Training Video", icon: Video, showFor: ["admin", "user"] },
  ];

  const accessibleRoutes = routes.filter((r) =>
    r.showFor.includes(userRole || "user")
  );
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gradient-to-br from-blue-50 to-purple-50">
      ...
      <div className="flex-1 flex flex-col">
        <header className="h-16 px-6 border-b bg-white flex items-center">
          <div className="flex flex-col">
            ...
            {!showAnimation && headerAnimatedText && (
              <p className="text-lg font-bold bg-gradient-to-r from-blue-600 to-purple-600 text-transparent bg-clip-text">
                {headerAnimatedText} 👋
              </p>
            )}
          </div>
        ...
        <footer className="h-8 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-center text-xs flex items-center justify-center">
          Powered by <span className="font-semibold ml-1">Botivate</span>
        </footer>
      </div>
    </div>
  );
